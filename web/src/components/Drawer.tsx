...
export function Drawer({ title, subtitle, onClose, children }: { title: ReactNode; subtitle?: ReactNode; onClose: () => void; children: ReactNode }) {
  return (
    <aside className="absolute bottom-4 right-4 top-[200px] z-20 flex w-[420px] animate-fadeIn flex-col rounded-lg border border-ink-600 bg-ink-850 shadow-2xl" aria-label="Drawer">
      <header className="flex items-start justify-between gap-3 px-5 pb-2 pt-4">
        ...
        <button onClick={onClose} className="mt-0.5 text-ink-400 hover:text-ink-100" aria-label="Close">
          ✕
        </button>
      </header>
