...
export function Explore({ tx, block, contract, className, children }: { tx?: string; block?: number; contract?: boolean; className?: string; children?: ReactNode }) {
  ...
  return (
    <Link to={{ pathname, search: loc.search }} className={cx('text-ink-300 hover:text-accent', className)} aria-label="Open in explorer">
      {children ?? '↗'}
    </Link>
  );
}
...
export function Lock() {
  return (
    <span title="never on chain" className="inline-flex cursor-default text-ink-400" aria-label="never on chain">
...
export function LockBlock({ label }: { label: string }) {
  return (
    <div className="flex items-center justify-between py-1.5 text-[13px]">
      ...
      <span className="inline-flex items-center gap-2" title="never on chain">
